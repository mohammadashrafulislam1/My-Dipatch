...
const NotificationComp = ({ onCountChange = () => {} }) => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        console.log(`📨 Setting ${notificationsData.length} notifications, ${unreadCountData} unread`);
  ...
  useEffect(() => {
    ...
    if (user?._id) {
      console.log("🔄 User has ID, fetching notifications");
      fetchNotifications();
    } else {
  ...
  useEffect(() => {
    console.log("🔌 Socket setup for user:", user?._id);
    if (!user?._id) return;

    socket.on("connect", () => {
      console.log("✅ Socket connected, joining as:", user._id);
